program md
  ...
  subroutine main
    ...
    do i=1,30
      ...
      open(50,file="traj.inp")
      read(50,*) trajectories
      close(50)
      write (6,*) trajectories


      do j=1, trajectories !2000
 ...
 !-----------------------------------------------------------------
  subroutine seed_generator
    ...
    open(51,file="seed.inp")
    read(51,*) the_one_seed
    write(6,*) the_one_seed
    close(51)

    do i=1,size_seed
      seed(i)=the_one_seed*(i**3+1729*i)
    enddo
  ...
  !-----------------------------------------------------------------
   ...
   implicit none
      ...
      transmission_lower=transmission_lower/50.d0
      transmission_upper=transmission_upper/50.d0
      reflection_lower=reflection_lower/50.d0
      reflection_upper=reflection_upper/50.d0
...
!    enddo
  ...
  subroutine compute_energy
    implicit none
    energy=0.5*mass*sum(v*v) + potential(state)
	!print *, 'v', v, 'v*v', v*v, 'sum(v*v)', sum(v*v)
